        ...
        <div className="p-7 h-full bg-gradient-to-b from-purple-100 to-pink-50">
          <div className="flex items-start justify-between">
            <div className="flex items-center justify-center w-[25%] h-40">
              <div className="relative object-fit h-[80%] w-40 rounded-full">
                {profile && (
                  <img src={profile?.image} className="rounded-full border-4 border-white" />
                )}
              </div>
            </div>
            <div className="flex flex-col w-[70%] py-2">
              <div className="flex flex-col">
                <h1 className="text-2xl italic font-bold ">
                  {rename(profile?.firstName)} {rename(profile?.lastName)} {` , ${profile?.role}`}
                </h1>
          ...
          {work?.filter((option) => option?.enabled)?.length > 0 && (
            <div className="flex items-start justify-between mt-4">
              <div className="flex justify-center w-[25%]">
                <h1 className="text-gray-600 italic font-semibold">Work Experience</h1>
              </div>
              <div className="w-[70%]">
                {work
                  ...
                  .map((option) => (
                    <div key={option?._id} className="flex flex-col mb-2">
                      ...
                      <div className="italic text-[13.5px] text-gray-400 font-semibold tracking-widest mb-1">
                        {`${months[new Date(option?.from).getMonth() + 1]} ${new Date(
                          option?.from
                        ).getFullYear()} - ${
          ...
          {education?.filter((education) => education?.enabled)?.length > 0 && (
            <div className="flex items-start justify-between mt-4">
              <div className="flex justify-center w-[25%]">
                <h1 className="text-gray-600 italic font-semibold">Education</h1>
              </div>
              <div className="w-[70%]">
                {education
                  ?.filter((option) => option?.enabled === true)
                  .map((option) => (
                    <div key={option?._id} className="flex flex-col mb-2">
                      ...
                      <div className="italic text-[13.5px] text-gray-400 font-semibold tracking-widest mb-1">
                        ...
                        } ${new Date(option?.endDate).getFullYear()}, ${option?.gpa} CGPA`}
                      </div>
                      {option?.summary?.enabled && (
                        <div className="markdown text-[13px]">
                          <MarkdownRenderer>{option?.summary?.data}</MarkdownRenderer>
                  ...
                  .map((project) => (
                    <div key={project?._id} className="flex flex-col mb-2">
                      <div className="flex justify-between">
                        ...
                        <div className="italic text-[13.5px] text-gray-400 font-semibold tracking-widest mb-1">
                          {`${months[new Date(project?.from).getMonth() + 1]} ${new Date(
                            project?.from
                          ).getFullYear()} - ${
            ...
            <div className="flex items-start justify-between mt-4">
              ...
              <div className="w-[70%] grid grid-cols-4 gap-2">
                {hobbies
                  ...
                  .map((hobby) => (
                    <div key={hobby?._id}>
                      <h1 className="text-[14px] font-semibold">{hobby?.name}</h1>
                    </div>
                  ))}
              </div>
            </div>
            ...
            <div className="flex items-start justify-between mt-4">
              ...
              <div className="grid grid-cols-2 gap-2 w-[70%]">
                {skills
                  ...
                  .map((skill) => (
                    <div key={skill?._id} className="flex items-center justify-between mr-2">
                      <h1 className="text-[14px] font-semibold">{skill?.name}</h1>
          ...
          {languages?.filter((language) => language?.enabled)?.length > 0 && (
            <div className="flex items-start justify-between mt-4">
              <div className="flex justify-center w-[25%]">
                <h1 className="text-gray-600 italic font-semibold">Langugages</h1>
              </div>
              <div className="w-[70%] grid grid-cols-2 gap-2">
                {languages
                  ?.filter((language) => language?.enabled === true)
                  .map((language) => (
                    <div key={language?._id} className="flex items-center justify-between mr-2">
                      <h1 className="text-[14px] font-semibold">{language?.name}</h1>
                      <span className="text-[13px] italic text-gray-600 font-semibold">
                        {language?.fluency}
                      </span>
                    </div>
